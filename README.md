...
This codebase uses a straightforward **Controller → Service → Repository** pattern. The business logic lives under `services` and stays free of infrastructure details. Errors returned from that layer are converted into transport-specific responses via a centralised handler. This avoids large `switch` statements when mapping service errors and keeps debugging consistent across transports.
...
- **`pkg`** – reusable helpers like the structured logger, lifecycle utilities, path helpers and validation helpers. See `pkg/README.md` for details.
...
- **`infrastructure/enums`** – string based enumerations like `ServiceName` used across packages. See `infrastructure/enums/README.md`.
- **`infrastructure/supervisor`** – helpers for systemd based restarts. See `infrastructure/supervisor/README.md`.
- **`infrastructure/utils`** – miscellaneous helpers such as HTTP and JWT utilities. See `infrastructure/utils/README.md` for details.
...
sequenceDiagram
    ...
    Server->>OutAgg: build aggregator using auth token
...
The REST server exposes autogenerated OpenAPI documentation. Start the server and visit [`/docs`](http://localhost:8080/docs) to browse the Swagger UI.

Error responses are documented with examples for each error code.

To regenerate `docs/openapi.json` and `docs/openapi.yaml` without running the server:

```bash
# Generate the OpenAPI spec (uses `config.yaml` for version information, default `0.1rc`)
go run ./cmd/gen_docs
```
...
The DSN uses `multiStatements=true` so multiple SQL statements per migration run correctly. Dirty migrations are handled based on the `DirtyStrategy` enum set in `Database.DirtyStrategy` (`rerun` or `skip`) which defaults to `rerun`.
