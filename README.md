...
This codebase uses a straightforward **Controller → Service → Repository** pattern. The business logic lives under `services` and stays free of infrastructure details. Controllers send responses through a small [Huma](https://github.com/danielgtaylor/huma) helper that wraps successful bodies in a standard envelope and translates errors into RFC 7807 `ProblemDetail` structures. Error codes are inferred automatically so handlers stay concise and debugging remains consistent across transports.
...
visit [`/docs/public`](http://localhost:8080/docs/public) for the public
endpoints or [`/docs/internal`](http://localhost:8080/docs/internal) to browse
the full internal API. Component schemas can be downloaded from
`/docs/public/schema/{name}.json` or `/docs/internal/schema/{name}.json`, and
each JSON response includes a `Link` header pointing to the relevant schema for
client-side validation. The docs paths, HTML renderer (Stoplight Elements or
Swagger UI), and OpenAPI version (`3.0.3` by default) are configured under the
`OpenAPI` section of `config.yaml`. Schema download paths are derived from the
docs paths and rarely need adjusting.

Error and success responses are registered with examples for each code. A compile-time generator precomputes the error list for every handler so documentation and runtime inference avoid expensive AST walks. To refresh the table, run:

```bash
go generate ./server/rest/utils
```
